    ...
    Profile
        ...
        .then(profile => {
            Post
                .findById(req.params.id)
                .then(post => {
                    const isLikedByCurrentUser = post.likes.filter(like => like.user.toString() === req.user.id).length > 0;
        ...
        .then(profile => {
            Post
                ...
                .then(post => {
                    const isLikedByCurrentUser = post.likes.filter(like => like.user.toString() === req.user.id).length > 0;
                    if (!isLikedByCurrentUser) {
                        res.status(400).json({ alreadyliked: "You have not yet liked this post" })
                    } else {
                    //    Get remove index
...
router.delete("/comments/:id/:comment_id",passport.authenticate("jwt", {session: false}), (req,res) => {
    ...
    Post
        ...
        .then(post => {
            ...
            const commentExists = post.comments.filter(comment => comment._id.toString() === req.params.comment_id).length > 0;
            if(!commentExists) {
